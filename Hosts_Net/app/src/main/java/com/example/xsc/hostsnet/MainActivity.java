...
public class MainActivity extends AppCompatActivity
    ...
    int btn_color[] = new int[5];
    final List<View> final_viewlist = new ArrayList<View>();
                ...
                if(!search_btn_thread.isAlive()){
                    if(!webEdt.getText().toString().isEmpty()){
                        if(!((dianxin_btn.getCurrentTextColor() == -1)&&(duoxian_btn.getCurrentTextColor() == -1)&&(liantong_btn.getCurrentTextColor() == -1)&&(yidong_btn.getCurrentTextColor() == -1)&&(haiwai_btn.getCurrentTextColor() == -1))){
                            ...
                            selected_num = 0;
                            btn_color[0] = dianxin_btn.getCurrentTextColor();
                            btn_color[1] = duoxian_btn.getCurrentTextColor();
                            btn_color[2] = liantong_btn.getCurrentTextColor();
                            btn_color[3] = yidong_btn.getCurrentTextColor();
                            btn_color[4] = haiwai_btn.getCurrentTextColor();
                            for(int a = 0 ; a < 5 ; a++){
                                if(btn_color[a] != -1){
                                    viewList.add(selected_num,final_viewlist.get(a));
                                    selected[selected_num] = choose[a];
                                    selected_num++;
                                }
                            }
                            ...
                            if(itemList1_num != 0){
                                recyclerView1.setAdapter(new MyAdapter((ArrayList<Item>) itemList1));
                            }
                            if(itemList2_num != 0){
                                recyclerView2.setAdapter(new MyAdapter((ArrayList<Item>) itemList2));
                            }
                            if(itemList3_num != 0){
                                recyclerView3.setAdapter(new MyAdapter((ArrayList<Item>) itemList3));
                            }
                            if(itemList4_num != 0){
                                recyclerView4.setAdapter(new MyAdapter((ArrayList<Item>) itemList4));
                            }
                            if(itemList5_num != 0){
                                recyclerView5.setAdapter(new MyAdapter((ArrayList<Item>) itemList5));
                            }
...
//                            viewPager.setAdapter(new MyViewPagerAdapter(viewList));
    ...
    public void init(){
        ...
        //为ViewPager设置适配器
        viewPager.setAdapter(new MyViewPagerAdapter(viewList));
        final_viewlist.add(dianxin_view);
        final_viewlist.add(duoxian_view);
        final_viewlist.add(liantong_view);
        final_viewlist.add(yidong_view);
        final_viewlist.add(haiwai_view);
